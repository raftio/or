...
app.post("/bundles", async (c) => {
  ...
  const bundle = bundleStore.createBundle(data, tenantId);
  return c.json(bundle, 201);
});

app.get("/bundles/:id", (c) => {
  const id = c.req.param("id");
  const bundle = bundleStore.getBundle(id, getTenantId(c));
  if (!bundle) {
...
app.get("/bundles", (c) => {
  ...
  const bundles = bundleStore.getBundlesByTicket(ticketId, getTenantId(c));
