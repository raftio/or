...
**Status:** Draft  
**Summary:** Defines OR’s overall architecture (Control vs Execution plane), the end-to-end product flow (Intent → Execution → Evidence → Outcome → Feedback), actors, system boundaries, and the placement of core features in the flow.

---

## 1. Overview

This RFC establishes the architectural foundation for OR. It does not define APIs, schemas, or integration contracts (those are covered by later RFCs). It does:

- Separate **Control Plane** (OR) from **Execution Plane** (existing tools).
...
OR is a **control plane**. It does not replace development tools, project management, or code editors; it **orchestrates** them.

### 2.1 Control Plane (OR)
...
**Boundary:** The Execution Plane is not owned by OR. OR **coordinates and validates** it (e.g. by supplying execution bundles and consuming evidence).
...
flowchart LR
  ...
  subgraph orca [OR Control Plane]
...
|------|------|-------------|
| 1 | **Upstream** | PM/Product create Idea → Spec (Confluence/Notion) → Ticket (Jira/Linear). OR reads spec + ticket. Optionally, PM/Product interact with OR or AI at Idea/Spec (see §4.1). |
...
| 4 | **Evidence** | OR collects evidence (test results, coverage, CI logs) and **validates** it before human review. |
...
| Actor | Primary interaction with OR | Stage(s) |
|-------|-----------------------------------|----------|
...
| **System / Automation** | CI, Git, ticket/docs providers; OR syncs and validates | All |

### 4.2 PM/Product ↔ OR at Idea & Spec (Idea / Spec Assist)

PMs can interact with OR (or an AI layer) **before** a ticket exists:
...
This reduces upstream ambiguity and improves execution bundle quality. The touchpoint is shown in the diagram as **Idea / Spec Assist**. Scoping (e.g. chat/copilot in Confluence/Notion vs OR UI) may be part of RFC-001 follow-up or a dedicated RFC.

---

## 5. System Boundaries

| Boundary | In scope (OR) | Out of scope (Execution / external) |
|----------|---------------------|--------------------------------------|
...
| **User interface** | OR UI for bundles, evidence, traceability, outcomes; optional PM assist UI | IDE, Jira/Linear, Confluence/Notion, GitHub/GitLab native UIs |
| **Integrations** | Adapters to tickets, docs, Git, CI, IDE (read/write metadata and links) | OR does not host Git, run CI, or edit code |

---

## 6. Placement of Core Features in the Flow

OR’s six core features (from [orca.md](../orca.md)) plus Idea/Spec Assist are placed as follows. Each will be detailed in a later RFC.

| Feature | Placement in flow | RFC |
|---------|-------------------|-----|
| **Idea / Spec Assist** | Upstream (Idea, Spec); optional PM ↔ OR/AI | This RFC; optional dedicated RFC |
...
| **Evidence Sync Engine** | Execution → OR; unified evidence view | RFC-017 |
